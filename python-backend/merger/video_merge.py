...
# 再次确保 ANTIALIAS 存在（部分环境在导入 moviepy 前未补丁到位）
try:  # pragma: no cover - 兼容性补丁
...
from merger.utils import adjust_output_path_extension, get_temp_dir
...
def _trim_clip_if_needed(
    ...
    """
    可选裁剪：根据 trim_seconds 与 trim_mode 截取片段。
    """
    ...
    if trim_seconds > duration:
        logger.warning("裁剪秒数 %.2f 超过视频时长 %.2f，已自动截到视频末尾。", trim_seconds, duration)
    ...
    logger.info("截取视频开头 %.2f 秒。", actual)
...
def merge_videos(
    ...
    if tail_image_path and tail_duration and tail_duration > 0:
        if not os.path.exists(tail_image_path):
            ...
            logger.info("追加尾帧图片 %s，停留 %.2f 秒。", tail_image_path, tail_duration)
...
def export_video_clip(video_clip: VideoFileClip, output_path: str, output_format: str, logger) -> str:
    """
    将 VideoClip 导出到指定路径，使用临时音频文件放在系统临时目录。
    """
    output_path = adjust_output_path_extension(output_path, output_format)
    os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
    temp_audiofile = os.path.join(get_temp_dir(), f"temp-audio-{uuid4().hex}.m4a")
