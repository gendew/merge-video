...
def prepare_voice_audio(
    ...
    """
    加载外部配音或根据文本生成 TTS，返回 AudioSegment。
    """
    ...
    if text_file:
        ...
        temp_audio = None
        try:
            temp_audio = reserve_temp_file(suffix=".wav")
            # 预留路径后直接由 pyttsx3 写入
...
def apply_voice_to_video(
    ...
    """
    将配音按指定策略叠加到视频，返回更新后的 clip 与临时音频路径。
    """
    ...
    # 使用 moviepy 的 audio_fadeout 确保尾部淡出（默认 1s，最长不超过视频时长）
